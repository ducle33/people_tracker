...
class sensor_integration(object):
    ...
    def leg_callback(self, data):
        self.pub_clusters(self.legPose)
        '''
    ...
    def yolo_callback(self, data): # if yolo and leg, then integrate; if only yolo then take the yolo
        ...
        for i in range(len(self.yoloPose.poses)):
            ...
            yoloArray.append([x_yolo, y_yolo, id_yolo])
        ...
        if len(self.legPose.poses):
            for i in range(len(self.legPose.poses)):   
                ...
                legArray.append([x_leg, y_leg, id_leg])
        
        
                    
        for i in range(len(self.yoloPose.poses)):
            result_pose = PoseWithCov()
            min_dist = 0.5 # Hall Model: R = 0.5m
            id_yolo = -1
            id_leg = -1
            found_leg = False
            self.marker_id = 0
            count_result = 0
            for j in range(len(self.legPose.poses)):
                if math.sqrt(pow(yoloArray[i][0] - legArray[j][0],2) + pow(yoloArray[i][1] - legArray[j][1],2)) < min_dist:
        ...
        if count_result > 0:
            self.pub_clusters(tracked_people)
