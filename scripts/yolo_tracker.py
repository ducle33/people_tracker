...
class KalmanMultiTracker:    
    ...
    def __init__(self):      
        ...
        self.max_leg_pairing_dist = rospy.get_param("max_leg_pairing_dist", 0.9)
        ...
        self.dist_travelled_together_to_initiate_leg_pair = rospy.get_param("dist_travelled_together_to_initiate_leg_pair", 0.001)
        ...
        # ROS subscribers         
        self.detected_clusters_sub = rospy.Subscriber('yolo_tracked', LegArray, self.detected_clusters_callback)      
        self.local_map_sub = rospy.Subscriber('local_map', OccupancyGrid, self.local_map_callback)

    	# ROS publishers
        self.yolo_tracked_pub = rospy.Publisher('yolo_tracking', PersonArray, queue_size=1000)
        self.yolo_detected_pub = rospy.Publisher('yolo_detected', PersonArray, queue_size=1000)
        
        self.yolo_pose_pub = rospy.Publisher('yolo_pose', PoseWithCovArray, queue_size=1000)

        self.marker_pub = rospy.Publisher('yolo_marker', Marker, queue_size=1000)
    ...
    def publish_tracked_people(self, now):
        ...
        self.yolo_tracked_pub.publish(people_tracked_msg)  
        
        # Publish yolo_pose message
        self.yolo_pose_pub.publish(pose_msg)          


if __name__ == '__main__':
    rospy.init_node('multi_yolo_tracker', anonymous=True)
